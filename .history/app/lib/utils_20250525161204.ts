...
export function histogramGrid(quality : string, map: (x: number, y: number) => [number, number], iterations: number) {
    let width: number;
    let height: number;
    ...
    for (let i : number = 0; i < iterations; i++) {
        position = temp[i];
        normalisedPosition = [clamp(Math.round(position[0] * scaleFactor + width/2), 0, width-1), clamp(Math.round(position[1] * scaleFactor + height/2), 0, height-1)];
        histogram[normalisedPosition[0]][normalisedPosition[1]]++;
    }
    return histogram;
}
